...
void encontrarExtremos(DEQUE* d){
  int min;
  int max;
  if (encontrarMinMax(d, &min, &max)){
    printf("Menor chave: %i ; maior chave: %i\n", min, max);
  }else{
    printf("Nao foi possivel encontrar a menor e a maior chave (DEQUE deve estar vazio).\n");
  }
}
...
void help(){
  ...
  printf("   x : encontrar a menor e a maior chave\n");
  printf("   q : sair\n");
}
